...
const usePicker = () => {
  ...
  const updatePickerDay = (
    day: Date | NepaliDate | null,
    programatic?: boolean
  ) => {
    const { onSelect } = pickerContextValue.pickerState;
    if (onSelect && programatic && day) {
  ...
  const updatePickerVisiblity = (newVis: boolean) => {
    setPickerState((prevPickerState) => {
      ...
      if (wasVisible === newVis) return prevPickerState;
  ...
  const togglePickerMode = (
    toggleIf: tpickerContextType["pickerState"]["mode"],
    defaultMode: tpickerContextType["pickerState"]["mode"]
  ) => {
    setPickerState((prevState) => {
      return {
        ...prevState,
  ...
  const updatePickerMode = (
    newMode: tpickerContextType["pickerState"]["mode"]
  ) => {
    setPickerState((prevState) => {
      return {
        ...prevState,
  ...
  const updatePickerYear = (
    newYear: tpickerContextType["pickerState"]["activeYear"]
  ) => {
    setPickerState((prevState) => {
      return {
        ...prevState,
  ...
  const changePickerLocale = (newLocale: "en" | "ne") => {
    setPickerState((prevState) => {
      if (prevState.locale === newLocale) return prevState;
      ...
      const updatedDate =
        newLocale === "ne"
          ? selectedDate instanceof NepaliDate
            ? selectedDate
            : NepaliDate.fromADDate(selectedDate as Date)
          : selectedDate instanceof Date
          ? selectedDate
          : (selectedDate as NepaliDate).toADDate();
  ...
  const canNavigateToPreviousMonth = (): boolean => {
    ...
    if (locale === "ne") {
      ...
      lastDayOfPrevMonth = new NepaliDate(
        prevYear,
        prevMonth,
        daysInMonth
      ).toADDate();
...
const PickerProvider = ({
  ...
  const getInitialActiveMonthYear = () => {
    ...
    if (!defaultDate) {
      ...
      if (minDate || maxDate) {
        ...
        if (!isTodayValid && minDate) {
          dateToUse =
            minDate instanceof NepaliDate ? minDate.toADDate() : minDate;
        }
      }
    ...
    if (defaultLocale === "BS") {
      ...
      const bsDate =
        dateToUse instanceof NepaliDate
          ? dateToUse
  ...
  const [pickerState, setPickerState] = useState<
    tpickerContextType["pickerState"]
  >({
