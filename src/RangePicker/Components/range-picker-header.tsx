...
const RangePickerHeader = ({ panel }: RangePickerHeaderProps) => {
  ...
  const panelState =
    panel === "left" ? rangePickerState.leftPanel : rangePickerState.rightPanel;
  const otherPanelState =
    panel === "left" ? rangePickerState.rightPanel : rangePickerState.leftPanel;
  ...
  const isUnsupportedYear = () => {
    if (
      ...
      activeYear === MAX_AD_YEAR
    ) {
      return true;
    }
    if (
      (locale === "ne" && activeYear === MIN_BS_YEAR) ||
      activeYear === MAX_BS_YEAR
    ) {
      return true;
    }
    return false;
  ...
  const isRightArrowDisabled = useMemo(() => {
    ...
    if (panel === "left") {
      ...
      if (
        nextYear === otherPanelState.activeYear &&
        adjustedNextMonth >= otherPanelState.activeMonth
      ) {
        return true;
      }
    ...
    activeMonth,
    activeYear,
    otherPanelState.activeMonth,
    otherPanelState.activeYear,
    isSinglePanel,
    canNavigateToNextMonth,
  ]);
  ...
  const isLeftArrowDisabled = useMemo(() => {
    ...
    if (panel === "right") {
      ...
      if (
        prevYear === otherPanelState.activeYear &&
        adjustedPrevMonth <= otherPanelState.activeMonth
      ) {
        return true;
      }
