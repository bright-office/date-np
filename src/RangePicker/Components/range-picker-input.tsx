...
const RangePickerInput = forwardRef<HTMLDivElement, RangePickerInputProps>(
    ({ className, placeholder = 'Select Date..',dateFormat, editable = false }, ref) => {
        ...
        const { handleInputChange, errors } = editable ? editableHook : { 
            handleInputChange: () => {}, 
        ...
        const handleStartDateInput = useCallback((e: React.FormEvent<HTMLDivElement>) => {
            if (!editable) return;
        ...
        const handleEndDateInput = useCallback((e: React.FormEvent<HTMLDivElement>) => {
            if (!editable) return;
            const target = e.currentTarget;
            const value = target.textContent || '';
        ...
        const handleKeyDown = useCallback((e: React.KeyboardEvent, type: 'start' | 'end') => {
            if (!editable) return;
            if (e.key === 'Enter') {
                e.preventDefault();
