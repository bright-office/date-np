...
const PickerInput = React.forwardRef<tpickerInputImperativeProps, tpickerInputProps>((props, ref) => {
    ...
    const { updatePickerVisiblity, pickerState, getDisplayDate, updatePickerDay } = usePicker();
    ...
    const { handleInputChange, error } = editable ? editableHook : { 
        handleInputChange: () => {}, 
        error: null 
    };
    ...
    const handleDateInput = useCallback((e: React.FormEvent<HTMLDivElement>) => {
        if (!editable) return;
        ...
        return (
            <div className="inputContainer">
                ...
                <div
                    ref={ref}
                    className={cn(
                        "relative flex h-[38px] items-center",
                        className
                    )}
                ...
                >
                    <div className="flex items-center">
                        ...
                        >
                            {displayDate ? formatDate(displayDate) : (inputProps.placeholder || "YYYY-MM-DD")}
                        </div>
                    </div>
