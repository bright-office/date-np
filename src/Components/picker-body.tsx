...
const PickerBody = ({
  ...
  const thisMonthtotalDays = getTotalDaysInMonth({
    date: currentMonthDate,
    locale,
  });
  const thisMonthStartDay = getStartingDayOfMonth({
    date: currentMonthDate,
    locale,
  });
  const prevMonthDate = createDate(activeYear, activeMonth - 1, 1);
  const prevMonthTotalDays = getTotalDaysInMonth({
    date: prevMonthDate,
    locale,
  });
  ...
  const TrailingPrevMonthDays = () => {
    return [...Array(plotablePrevMonthDays)].map((_, index) => {
      ...
      const isNotActive =
        !selectedDate || !areDatesEqual(dayDate, selectedDate);
  ...
  const DatePickerBody = () => {
    if (pickerState.mode !== "date") return null;
  ...
  const MonthPickerBody = () => {
    if (pickerState.mode !== "month") return null;
  ...
  const YearPickerBody = () => {
    if (pickerState.mode !== "year") return null;
    ...
    return (
      <div
        ...
        ref={(el) => {
          if (el) {
            const activeYearIndex = activeYear - minYear;
            const itemsPerRow = 3;
            const rowHeight = 44;
            const containerHeight = 288;

            const activeYearRow = Math.floor(activeYearIndex / itemsPerRow);
            const targetScrollTop =
              activeYearRow * rowHeight - containerHeight / 2 + rowHeight / 2;
